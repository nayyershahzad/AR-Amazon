...
export class BehavioralAnalysisService {
  ...
  static async analyzeWithAI(userId: string, financialData: any, simulatorData?: any): Promise<{
    profile: BehavioralProfile;
    insights: BehavioralInsight[];
    nudges: BehavioralNudge[];
  }> {
    console.log('🧠 Starting AI-powered behavioral analysis for user:', userId);

    try {
      // Define the schema for AI response
      const BehavioralAnalysisSchema = z.object({
        personalityAssessment: z.object({
          type: z.enum(['spender', 'saver', 'balanced', 'impulsive', 'analytical']),
          confidence: z.number().min(0).max(100),
          reasoning: z.string(),
          characteristics: z.array(z.string())
        }),
        motivationType: z.enum(['visual', 'social', 'milestone', 'competitive', 'educational']),
        riskTolerance: z.enum(['conservative', 'moderate', 'aggressive']),
        spendingPatterns: z.array(z.object({
          pattern: z.string(),
          trigger: z.enum(['emotional', 'temporal', 'situational', 'social']),
          frequency: z.number(),
        ...
        paymentBehavior: z.object({
          ...
          reliability: z.enum(['excellent', 'good', 'needs_improvement']),
          patterns: z.array(z.string()),
          recommendations: z.array(z.string())
        }),
        behavioralInsights: z.array(z.object({
          type: z.enum(['spending_pattern', 'payment_behavior', 'motivation_trend', 'risk_factor', 'opportunity']),
          title: z.string(),
        ...
        personalizedNudges: z.array(z.object({
          type: z.enum(['spending_prevention', 'payment_motivation', 'habit_formation', 'goal_reinforcement']),
          title: z.string(),
          message: z.string(),
          priority: z.enum(['low', 'medium', 'high']),
          actionText: z.string(),
          emotionalTone: z.enum(['encouraging', 'warning', 'celebratory', 'educational'])
        }))
      });

      // Prepare the prompt for AI analysis
      const analysisPrompt = `
You are an expert financial behavioral analyst. Analyze this user's financial and payment behavior data to provide deep psychological insights and actionable recommendations.

## Financial Data:
- Monthly Income: $${financialData.totalIncome?.toLocaleString() || 'N/A'}
- Monthly Expenses: $${financialData.totalExpenses?.toLocaleString() || 'N/A'}
- Available for Debt: $${financialData.availableForDebt?.toLocaleString() || 'N/A'}
- Total Debt: $${financialData.debts?.reduce((sum: number, d: any) => sum + d.balance, 0).toLocaleString() || 'N/A'}
- Number of Debts: ${financialData.debts?.length || 0}
- Top Expense Category: ${financialData.expenses?.reduce((max: any, exp: any) => exp.amount > max.amount ? exp : max, {category: 'Unknown', amount: 0})?.category || 'Unknown'}
- Financial Goals: ${financialData.goals?.length || 0} active goals

## Payment Simulator Behavior (if available):
${simulatorData ? `
- Current Month: ${simulatorData.currentMonth || 'N/A'}
- Total Payments Made: ${simulatorData.paymentHistory?.length || 0}
- Skipped Payments: ${simulatorData.paymentHistory?.filter((p: any) => p.action === 'skip').length || 0}
- Extra Payments: ${simulatorData.paymentHistory?.filter((p: any) => p.action === 'advance').length || 0}
- Delinquent Accounts: ${simulatorData.delinquencyData?.accountsDelinquent || 0}
- Past Due Amount: $${simulatorData.delinquencyData?.totalPastDue?.toLocaleString() || '0'}
- Payment Consistency: ${simulatorData.paymentHistory?.length > 0 ? Math.round(((simulatorData.paymentHistory.length - (simulatorData.paymentHistory?.filter((p: any) => p.action === 'skip').length || 0)) / simulatorData.paymentHistory.length) * 100) : 0}%
` : 'No payment simulator data available'}

Based on this comprehensive financial and behavioral data, provide:

1. **Personality Assessment**: Determine their financial personality type based on spending patterns, income utilization, and payment behavior
2. **Motivation Type**: What motivates this person most effectively
3. **Risk Tolerance**: Based on their financial decisions and debt management approach
4. **Spending Patterns**: Identify concerning patterns and triggers
5. **Payment Behavior Analysis**: Evaluate their payment reliability and consistency
6. **Behavioral Insights**: 3-5 key insights about their financial behavior
7. **Personalized Nudges**: 3-4 highly personalized behavioral nudges that would effectively guide this specific user

Focus on:
- Psychology behind their financial decisions
- Patterns that indicate stress or positive behavior
...
`;
      ...
      console.log('🤖 Sending analysis request to Google Gen AI...');
      
      // Call Google Gen AI for analysis
      const llmResponse = await generate({
        model: gemini15Flash,
        prompt: analysisPrompt,
        output: {
          schema: BehavioralAnalysisSchema,
        },
      });

      const aiAnalysis = llmResponse.output();
      console.log('✅ Received AI analysis response');

      if (!aiAnalysis) {
        throw new Error('AI analysis returned null response');
      }

      // Convert AI analysis to our internal format
      const profile: BehavioralProfile = {
        userId,
        personalityType: aiAnalysis.personalityAssessment.type,
        motivationType: aiAnalysis.motivationType,
        riskTolerance: aiAnalysis.riskTolerance,
        spendingTriggers: aiAnalysis.spendingPatterns.map((pattern, index) => ({
          id: `ai_trigger_${Date.now()}_${index}`,
          type: pattern.trigger,
          trigger: pattern.pattern,
          frequency: pattern.frequency,
          averageAmount: pattern.averageAmount,
          timePattern: 'AI-detected pattern',
          confidence: 0.85,
          lastOccurrence: new Date()
        })),
        paymentPatterns: [{
          id: `ai_pattern_${Date.now()}`,
          type: aiAnalysis.paymentBehavior.reliability === 'excellent' ? 'consistent' : 
                aiAnalysis.paymentBehavior.reliability === 'good' ? 'irregular' : 'stress-driven',
          frequency: 'monthly',
          timing: 'AI-analyzed timing',
          reliability: aiAnalysis.paymentBehavior.consistency,
          averageAmount: financialData.availableForDebt || 0,
          emotionalState: 'neutral'
        }],
        ...
        successFactors: [
          {
            ...
            factor: aiAnalysis.personalityAssessment.characteristics[0] || 'Consistent behavior',
            type: 'internal',
            importance: 85,
            currentLevel: aiAnalysis.paymentBehavior.consistency,
            improvement: aiAnalysis.paymentBehavior.recommendations[0] || 'Continue current approach'
          }
        ],
        lastAnalyzed: new Date(),
        confidenceScore: aiAnalysis.personalityAssessment.confidence
      };

      // Convert AI insights to our format
      const insights: BehavioralInsight[] = aiAnalysis.behavioralInsights.map((insight, index) => ({
      ...
      // Convert AI nudges to our format
      const nudges: BehavioralNudge[] = aiAnalysis.personalizedNudges.map((nudge, index) => ({
        id: `ai_nudge_${Date.now()}_${index}`,
        userId,
        type: nudge.type,
        content: {
          title: nudge.title,
        ...
        trigger: { event: 'manual_trigger', condition: 'manual', threshold: 0 },
        timing: { 
          deliveryTime: new Date(), 
          expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000),
          optimalWindow: { start: '09:00', end: '17:00' },
          timezone: 'UTC',
      ...
      this.insights.set(userId, insights);
      const userNudges = this.nudgeQueue.get(userId) || [];
      this.nudgeQueue.set(userId, [...userNudges, ...nudges]);

      console.log(`✅ AI behavioral analysis completed for user ${userId}`);
      console.log(`📊 Generated: Profile (${profile.personalityType}), ${insights.length} insights, ${nudges.length} nudges`);
      ...
      const fallbackProfile = await this.createDemoProfile(userId);
      const fallbackInsights = await this.getUserInsights(userId);
      const fallbackNudges = await this.getPendingNudges(userId);
      
      return {
        profile: fallbackProfile,
        insights: fallbackInsights,
        nudges: fallbackNudges
      };
    }
  }
